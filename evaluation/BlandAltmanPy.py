...
class BlandAltman():
    ...
    def scatter_plot(self,x_label='Gold Standard',y_label='New Measure',
        ...
        plt.savefig(os.path.join(self.save_path, file_name),bbox_inches='tight', dpi=300, format='png')
    ...
    def difference_plot(self,x_label='Difference between rPPG HR and ECG HR [bpm]',
        ...
        if show_legend:
            ax.legend()
        ax.grid()
        plt.savefig(os.path.join(self.save_path, file_name),bbox_inches='tight', dpi=100, format='png')
        print(f"Saved {file_name} to {self.save_path}.")
