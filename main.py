...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'My':#注意力機制
        model_trainer = trainer.MultiPhysNetTrainer2.MultiPhysNetTrainer(config, data_loader_dict)   
    elif config.MODEL.NAME == 'Res':#殘差特徵
        model_trainer = trainer.MultiPhysNetTrainer1.MultiPhysNetTrainer(config, data_loader_dict) 
    elif config.MODEL.NAME == 'rawFCA':#殘差特徵
        model_trainer = trainer.MultiPhysNetTrainer0.MultiPhysNetTrainer(config, data_loader_dict)     
    elif config.MODEL.NAME == 'Trans':
        model_trainer = trainer.FactorizePhysTrainer2.FactorizePhysTrainer(config, data_loader_dict) 
    elif config.MODEL.NAME == 'Ad':
        model_trainer = trainer.MultiPhysNetTrainer3.MultiPhysNetTrainer(config, data_loader_dict) 
    elif config.MODEL.NAME == 'Dub':
        model_trainer = trainer.BvpSpo2Trainer.BvpSpo2Trainer(config, data_loader_dict)  
    else:
...
def test(config, data_loader_dict):
    ...
    if config.MODEL.NAME == "FCA":
        ...
        model_trainer = trainer.FactorizePhysTrainer.FactorizePhysTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'My':
        model_trainer = trainer.MultiPhysNetTrainer2.MultiPhysNetTrainer(config, data_loader_dict)  
    elif config.MODEL.NAME == 'Res':#殘差特徵
        model_trainer = trainer.MultiPhysNetTrainer1.MultiPhysNetTrainer(config, data_loader_dict) 
    elif config.MODEL.NAME == 'rawFCA':
        model_trainer = trainer.MultiPhysNetTrainer0.MultiPhysNetTrainer(config, data_loader_dict)     
    elif config.MODEL.NAME == 'Trans':    
        model_trainer = trainer.FactorizePhysTrainer2.FactorizePhysTrainer(config, data_loader_dict) 
    elif config.MODEL.NAME == 'Ad':
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        elif config.TRAIN.DATA.DATASET == "Spo22":
            train_loader = data_loader.Spo2Loader0.Spo2Loader
        else:
        ...
        elif config.VALID.DATA.DATASET == "Spo22":
            valid_loader = data_loader.Spo2Loader0.Spo2Loader            
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        elif config.TEST.DATA.DATASET == "Spo22":
            test_loader = data_loader.Spo2Loader0.Spo2Loader            
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS and iBVP.")
        ...
        if config.UNSUPERVISED.DATA.DATASET == "UBFC":
            ...
            unsupervised_loader = data_loader.PURELoader0.PURELoader
        ...
        elif config.UNSUPERVISED.DATA.DATASET == "Spo22":
            unsupervised_loader = data_loader.Spo2Loader0.Spo2Loader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
