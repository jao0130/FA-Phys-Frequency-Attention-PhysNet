...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data):
        ...
        self.inputs_finger = list() # finger
        ...
        if config_data.DO_PREPROCESS:
            ...
            print("self.raw_data_dirs:",self.raw_data_dirs)   # [0, 1]
            self.preprocess_dataset(self.raw_data_dirs, config_data.PREPROCESS, config_data.BEGIN, config_data.END) # 两步
            # self.preprocess_dataset(self.raw_data_dirs[1], config_data.PREPROCESS, config_data.BEGIN, config_data.END) # 两步
        else:
    ...
    def __getitem__(self, index):
        ...
        # NCDHW
        if self.dataset_type == "face": # single
        ...
        split_idx = item_path_filename.rindex('_input')
        filename = item_path_filename[:split_idx]
        chunk_id = item_path_filename[split_idx + len('_input'):].split('.')[0]
        #print(f"item_path_filename: {item_path_filename}, item_path_filename_2: {item_path_filename_2}")
        # print(f"filename: {filename}")
        if self.dataset_type == "both": # 多一个data
            return data_face, data_finger, label_bvp, label_spo2, filename, chunk_id
        else:
             return data, label_bvp, label_spo2, filename, chunk_id
            
    def _detrend(self, input_signal):
        """Detrend the input signal using sparse matrix operations."""
        
        lambda_value = 5 # 10~150
        window_size = 42

        signal_length = input_signal.shape[0]
        H = identity(signal_length, format='csr')
        ones = np.ones(signal_length)
        minus_twos = -2 * np.ones(signal_length)
        diags_data = np.array([ones, minus_twos, ones])
        diags_index = np.array([0, 1, 2])
        D = spdiags(diags_data, diags_index, signal_length - 2, signal_length)
        A = H + (lambda_value ** 2) * (D.T @ D)

        signal = spsolve(A, input_signal)

        baseline = np.convolve(signal, np.ones(window_size) / window_size, mode='same')
        amplitude_center = np.median(signal)
        corrected_signal = signal - baseline
        corrected_signal += amplitude_center
    ...
    def baseline_correction(signal, window_size):
        """
        基線校正：使用滑動窗口平均來估計基線，並將其從原始信號中減去。
        
        :param signal: 原始信號（BVP）
        :param window_size: 滑動窗口大小（以採樣點數為單位）
        :return: 校正後的信號
        """
        # 計算滑動窗口平均
        baseline = np.convolve(signal, np.ones(window_size) / window_size, mode='same')
    ...
    @staticmethod
    def _bandpass_filter(signal, fs, lowcut=0.5, highcut=4.0, order=4):
    ...
    def preprocess(self, frames, bvps, config_preprocess):
        ...
        if config_preprocess.DO_CHUNK:  # chunk data into snippets
            frames_clips, bvps_clips = self.chunk_raw(
    ...
    def preprocess0(self, frames, bvps, spo2, config_preprocess, video_type):
        """Preprocesses a pair of data."""
        pass


    def chunk_raw(self, frames, bvps, chunk_length):
    ...
    def chunk(self, frames, bvps, spo2, chunk_length, video_type):
        """Chunk the data into small chunks.
        ...
        return np.array(frames_clips), np.array(bvps_clips) if bvps_clips is not None else None,\
              np.array(spo2_clips) if spo2_clips is not None else None

    def chunk_overlap(self, frames, bvps, spo2, chunk_length, video_type):
        """Chunk the data into small chunks with overlap.
        ...
        Returns:
            ...
            spo2_clips: all chunks of spo2 frames (None for finger)
        """
        overlap = chunk_length // 4  # 設置重疊長度，例如 50% 重疊
        stride = chunk_length - overlap  # 步幅，確保重疊
        clip_num = max(1, (frames.shape[0] - chunk_length) // stride + 1)  # 計算可能的 chunk 數量

        frames_clips = [frames[i * stride:i * stride + chunk_length] for i in range(clip_num) 
                    if i * stride + chunk_length <= frames.shape[0]]
        
        if video_type == "face":
            bvps_clips = [bvps[i * stride:i * stride + chunk_length] for i in range(clip_num) 
                        if i * stride + chunk_length <= frames.shape[0]]
            spo2_clips = [spo2[i * stride:i * stride + chunk_length] for i in range(clip_num) 
                        if i * stride + chunk_length <= frames.shape[0]]
        else:
            bvps_clips = None
            spo2_clips = None
    ...
    def chunk_only_bvp(self, frames, bvps, chunk_length, video_type):
        """Chunk the data into small chunks with overlap.
    ...
    def save_multi_process(self, frames_clips, bvps_clips, spo2_clips, filename, clip_idx):
    ...
    def load_preprocessed_data(self):
        ...
        # print(f"labels_spo2:{labels_spo2}")
        self.inputs = inputs
        # print(f"self.inputs: {self.inputs}")
        self.labels_bvp = labels_bvp
        self.labels_spo2 = labels_spo2
        print(f"inputs_length: {len(inputs)}, labels_bvp_length: {len(labels_bvp)}, labels_bvp_length: {len(labels_spo2)}")
