...
from evaluation.self_correlation import compute_self_correlation
...
class SUMSLoader(BaseLoader):
    ...
    def preprocess_dataset_subprocess(self, data_dirs, config_preprocess, i,  file_list_dict):
        ...
        experiment_id = video_dir.split(os.sep)[-1]
        ...
        resampled_bvp = self._bandpass_filter(
            resampled_bvp, fs=self.config_data.FS, lowcut=0.5, highcut=4.0
        )
        ...
        if config_preprocess.USE_PSUEDO_PPG_LABEL:
            frames_gen = self.read_video(video_file, chunk_size=1184)
            all_frames = []
            for chunk in frames_gen:
        ...
        for frames_chunk in frames_gen:
            # 截取對應的標籤
            start_frame = chunk_idx * 1184
            end_frame = start_frame + frames_chunk.shape[0]
            bvps_chunk = bvps[start_frame:end_frame] if bvps is not None else None
            spo2_chunk = spo2[start_frame:end_frame] if spo2 is not None else None

            if "face" in video_file:  # face
                frames_clips, bvps_clips, spo2_clips = self.preprocess(
                    frames_chunk, bvps_chunk, spo2_chunk, config_preprocess, "face"
                )
                filename = f"{subject_id}_{experiment_id}"
                input_names, label_names, spo2_names = self.save_multi_process(
                    frames_clips, bvps_clips, spo2_clips, filename, chunk_idx
                )
                input_name_list.extend(input_names)
                label_name_list.extend(label_names)
                spo2_name_list.extend(spo2_names)
               
            else:  # finger
                frames_clips, _, _ = self.preprocess(
                    frames_chunk, None, None, config_preprocess, "finger"
                )
                filename = f"{subject_id}_{experiment_id}_finger"
                input_names = self.save_multi_process_no_labels(frames_clips, filename, chunk_idx)
                input_name_list.extend(input_names)

            chunk_idx += 1  # 累進 chunk 索引
    ...
    def read_hr(hr_file):
        ...
        return timestamps, hr_values

    @staticmethod
    def read_frame_timestamps0(timestamp_file):
        """Reads timestamps for each video frame and resamples to match 30 FPS."""
        data = pd.read_csv(timestamp_file)
        timestamps = data['timestamp'].values
        frame_interval = 2

        sampled_timestamps = timestamps[::frame_interval]
    ...
    @staticmethod
    def read_video(video_file, chunk_size=600):
    ...
    def read_video0(video_file):
        ...
        frames = []
        target_size = (640, 480)
        input_width = int(VidObj.get(cv2.CAP_PROP_FRAME_WIDTH))
        input_height = int(VidObj.get(cv2.CAP_PROP_FRAME_HEIGHT))
        need_resize = input_width != target_size[0] or input_height != target_size[1]
    ...
    def preprocess(self, frames, bvps, spo2, config_preprocess, video_type):
        ...
        if bvps is not None and spo2 is not None:
            ...
            if config_preprocess.DO_CHUNK:
                frames_clips, bvps_clips, spo2_clips = self.chunk_overlap(data, bvps, spo2, config_preprocess.CHUNK_LENGTH, video_type)
            else:
            ...
            if config_preprocess.DO_CHUNK:
                frames_clips, _, _ = self.chunk_overlap(data, None, None, config_preprocess.CHUNK_LENGTH, video_type)
            else:
                frames_clips = np.array([data])
    ...
    def save_multi_process_no_labels(self, frames_clips, filename, clip_idx):
        """Save all the chunked data with multi-thread processing (no labels for finger)."""
        if not os.path.exists(self.cached_path):
            os.makedirs(self.cached_path, exist_ok=True)
        
        input_path_name_list = []
        count = 12 * clip_idx  # 每個 chunk 有 12 個片段，計算起始索引
        for i in range(len(frames_clips)):
            input_path_name = os.path.join(self.cached_path, f"{filename}_input_{count}.npy")
            input_path_name_list.append(input_path_name)  # 追加路徑到列表
            np.save(input_path_name, frames_clips[i])  # 保存單個片段
