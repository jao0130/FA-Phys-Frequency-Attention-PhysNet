...
class Spo2Loader(BaseLoader):
    ...
    def get_raw_data(self, data_path):
        ...
        for data_dir in data_dirs:
            dir_name = os.path.split(data_dir)[-1]  # 獲取目錄名稱（例如 "p001"）
            try:
             # 移除 'p' 前綴並轉為整數（例如 "p001" → "001" → 1）
                subject = int(dir_name.lstrip('p'))
            except ValueError:
                raise ValueError(f"Invalid subject ID format: {dir_name}")
            
            d_dirs = os.listdir(data_dir)
            print(d_dirs)
            # v01 v02 v03 v04
            for dir in d_dirs:
                if dir in ['v01', 'v03', 'v04', 'v05']:    
                    items_dirs = os.listdir(data_dir + os.sep + dir) # avi csv 
                    for item in items_dirs:
                        if "avi" in item or "npy" in item: # If returning all together here
                            file_type = 'face' if 'MJPG' in item else 'finger'         
